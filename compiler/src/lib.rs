...
pub fn compile_with_path(source: &str, file_path: Option<&Path>) -> String {
    ...
    let mut program = parse_with_imports(source, base_dir.as_deref(), &mut HashSet::new());
...
fn parse_with_imports(source: &str, base_dir: Option<&Path>, visited: &mut HashSet<PathBuf>) -> Vec<ast::Decl> {
    ...
    for import_path in &imports_to_resolve {
        if let Some(ref base) = base_dir {
            let resolved_path = base.join(import_path);
            // Add .ido extension if not present
            let resolved_path = if resolved_path.extension().is_none() {
                resolved_path.with_extension("ido")
            } else {
                resolved_path
            };
            ...
            if resolved_path.exists() {
                ...
                eprintln!("Import not found: '{}'", resolved_path.display());
                std::process::exit(1);
            }
        } else {
            eprintln!("Cannot resolve import '{}' without a file path", import_path);
            std::process::exit(1);
        }
    }
